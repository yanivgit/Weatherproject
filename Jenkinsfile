...
'http://44.213.40.16/gitlab-instance-cab3d91b/project.git']])
	...
	stage('SSH Steps Rocks!') {
	    steps {
		sshagent(['sshUser']) {
		    sh 'ssh -o StrictHostKeyChecking=no -l ubuntu 3.95.200.101 echo $DOCKERHUB_CREDENTIALS_PSW |docker login -u $DOCKERHUB_CREDENTIALS_USR --password-stdin'
		    sh 'ssh -o StrictHostKeyChecking=no -l ubuntu 3.95.200.101 sudo docker pull avivlevari/project_image'
		    sh 'ssh -o StrictHostKeyChecking=no -l ubuntu 3.95.200.101 sudo docker stop $(sudo docker ps -aq) || true'
		    sh 'ssh -o StrictHostKeyChecking=no -l ubuntu 3.95.200.101 sudo docker rm $(sudo docker ps -aq) || true'
                    sh 'ssh -o StrictHostKeyChecking=no -l ubuntu 3.95.200.101 sudo docker-compose up -d'
